<template>
  <div class="therapeutist">
    <div class="therapinfo">
      <img
        class="teraphoto"
        v-bind:src="therapeutist[0].photoPhoto.photoSrc"
        :alt="'Avatar - '+ therapeutist[0].therapName + ' ' + therapeutist[0].therapSurname"
      />
    ...
    <div v-if="therapeutist.methods.length != 0" class="thermet">
      ...
      <ul>
        <router-link
          v-for="(method,index) in therapeutist.methods"
          v-if="method.metId.metBody!=''"
          :key="index"
          class="methodlink"
          :to="'/metoda/'+method.metId.metId"
        >
          <li>
            <h4>
              {{method.metId.metName}}
        ...
        <li
          v-for="method in therapeutist.methods"
          v-if="method.metId.metBody==''"
          class="methodlink"
        >
...
export default {
  ...
  methods: {
    fetchTherapist() {
      this.$http
        .get("/api/therapeutist/" + this.$route.params.id, {
          headers: {
        ...
        .then(result => {
          ...
          this.loaded = true;
          document.title =
            "RehaWałbrzych - terapeuta: " +
            this.therapeutist[0].therapName +
            " " +
            this.therapeutist[0].therapSurname;
          document.head.querySelector("meta[name=description]").content =
            "Terapeuta " +
            this.therapeutist[0].therapName +
            " " +
            this.therapeutist[0].therapSurname +
            ". RehaWałbrzych - rehabilitacja i integracja sensoryczna dzieci i niemowląt w Wałbrzychu.";
        });
    }
  },
  created: function() {
